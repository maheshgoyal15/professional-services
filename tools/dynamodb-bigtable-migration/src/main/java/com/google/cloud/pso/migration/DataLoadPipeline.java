...
public class DataLoadPipeline {
  ...
  public static void main(String[] args) throws Exception {
    DataLoadOptions options = PipelineOptionsFactory.fromArgs(args).withValidation().as(DataLoadOptions.class);
    ...
    if (options.getBigtableInstanceId() == null || options.getBigtableProjectId() == null || options.getBigtableTableId() == null) {
    ...
    ControlFileProcessor.ControlFileConfig config = ControlFileProcessor.parseControlFile(options.getControlFilePath());

    // 2. Read and Process Data using Control File Config
    // Priority: Use inputFilePath from command line if present, otherwise fall back to control file's sourceFileLocation
    String inputPath = options.getInputFilePath() != null ? options.getInputFilePath() : config.sourceFileLocation;
    if (inputPath == null) {
      throw new IllegalArgumentException("Input file path must be provided either via --inputFilePath or in the Control File.");
    }

    PCollection<Row> inputData = pipeline.apply(
        "Read from Cloud Storage",
        new ReadFromGCS(inputPath, InputFormat.DYNAMO, config));

    // 3. Write to Bigtable
    inputData.apply("Write To Bigtable", new WriteToBigtable(options.getBigtableMaxMutationsPerRow()));
